...
function StatementTypeBadge({ type, isDark }: { type: QueryHistoryEntry['statementType']; isDark: boolean }) {
    ...
    return <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 ${styles[type]}`}>{type}</span>;
}
...
function HistoryEntryRow({
    ...
    const truncatedSql = entry.sql.length > 80 ? entry.sql.slice(0, 80) + '…' : entry.sql;
    ...
    return (
        ...
        >
            ...
            {/* SQL preview */}
            <p
                className={`font-mono text-xs leading-relaxed break-all ${isDark ? 'text-neutral-200' : 'text-gray-800'}`}
            >
                {truncatedSql}
            </p>
                    ...
                    {entry.error ? (
                        ...
                        <>
                            ...
                            {entry.executionTime != null && (
                                <span className={isDark ? 'text-neutral-500' : 'text-gray-400'}>
    ...
    return (
        <div className={`flex h-full flex-col ${isDark ? 'bg-neutral-800' : 'bg-gray-50'}`}>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
                {sorted.length === 0 ? (
                    <p
                        className={`mt-6 text-center text-xs select-none ${isDark ? 'text-neutral-500' : 'text-gray-400'}`}
                    >
                        {entries.length === 0 ? 'No query history yet' : 'No results match your filter'}
                    </p>
                ) : (
                    sorted.map((entry) => (
                        <HistoryEntryRow
